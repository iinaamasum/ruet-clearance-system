...
const HallClearanceSelection = ({ navigation }) => {
  ...
  const onSubmit = (data) => {
    ...
    if (data.due > 0 && data.trx_id === 'NULL') {
      ...
      const { full_name, email, dept, roll, series, contact_number, faculty } =
        info;
      axios
        .post('http://localhost:5000/hall-clearance-application', {
          full_name,
          email,
          dept,
          roll,
      ...
      <View style={tw`px-5 mt-6`}>
        <View>
          ...
          <Text style={tw`text-gray-500 text-center mb-3 px-2`}>
            The application is directly sent to hall provost.
          </Text>
